...
const Template2 = (args) => {
  ...
  return (
    <Accordion>
        <Accordion.Button onClick={()=> open? setOpen(false):setOpen(true)} className={cn("flex justify-between  border-primary-500 ")}  iconOpen={<span className="font-icons-solid text-lg order-12">expand_less </span>} iconClosed={<span className="font-icons-solid text-lg order-12">expand_more </span>} open={open} variant="outlined">
       <div className="flex space-x-2.5  order-1 w-full"><span className="font-icons-outlined text-lg text-surface-300">person</span> <span className="font-bold font-texts">Title</span></div>
       <div className="flex space-x-2.5 order-last">
         <span className="font-texts">Text</span></div>
     </Accordion.Button>
     
     <Accordion.Panel static open={open}>
       Panel
     </Accordion.Panel>
   </Accordion>
