...
const  Items = (props:ElementType)=> {
  ...
  return (
      <Menu.Items  className={cn("p-1 rounded overflow-y-auto  absolute mt-2 left-0 shadow w-52 bg-surface-0 z-10 overflow-x-hidden", className)} {...restProps}>
         {children}     
      </Menu.Items>
  )
}
const  Item = (props:ElementType)=> {
  const {children}=props;
  return (
      <Menu.Item as="div" className="flex p-3 space-x-2.5 overflow-hidden hover:bg-surface-100 font-texts text-base w-full">
         {children}     
      </Menu.Item>
  )
}
