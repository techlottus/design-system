...
To get started, install `@bedu-tmp/datetime` via yarn:

```bash
# Yarn
yarn add @bedu-tmp/datetime
```
...
```jsx highlight=9
import { DateInput } from "@bedu-tmp/datetime";
...
import { DateInput } from "@bedu-tmp/datetime";
import { FormGroup } from "@bedu-tmp/betomic";
