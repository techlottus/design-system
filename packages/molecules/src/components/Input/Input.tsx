...
const Input: InputGroupType = (props: InputGroupType) => {
  ...
  //   e.preventDefault;
  //   let valueInput = e.target.inputName.value;
  //   setIvalue(valueInput);
  //   setValid(regexExp!=null && regexExp.test(valueInput))
  //   console.log(Ivalue)
  // }


  const inputClasses = cn("flex order-2 grow peer", "w-full", "border-transparent focus:outline-none pt-3 px-1",
  ...
  const labelClasses = cn("order-2 text-xs font-texts  absolute scale-75 origin-center duration-300 transform -translate-y-8 ",
    ...
    {["text-surface-800  peer-focus:text-surface-800"]:isValid==null  && !disabled,
    ["text-success-500  peer-focus:text-success-500"]:isValid  && !disabled,
    ["text-error-500  peer-focus:text-error-500"]:!isValid  && !disabled,
  ...
  {["border-surface-200 focus-within:border-surface-600"]:isValid==null && !disabled,
  ["border-success-500 focus-within:border-success-500"]:isValid  && !disabled,
  ["border-error-500 focus-within:border-error-500"]:!isValid  && !disabled,
...
//   const labelClasses = cn("order-2 text-xs font-texts  absolute scale-75 origin-center duration-300 transform -translate-y-8 ",
//     "peer-placeholder-shown:text-base peer-placeholder-shown:text-surface-700 peer-placeholder-shown:-translate-y-5 ",
//     "peer-focus:text-xs peer-focus:-translate-y-8 ",{["text-surface-800  peer-focus:text-surface-800"]:isValid==null  && !disabled,["text-success-500  peer-focus:text-success-500"]:isValid  && !disabled,["text-error-500  peer-focus:text-error-500"]:!isValid  && !disabled},{["text-surface-200  peer-placeholder-shown:text-surface-200 peer-placeholder-shown:text-surface-200"]:disabled});
